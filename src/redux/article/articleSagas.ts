...
import { redirectNotFoundPage } from 'utils';
import { getAnswersApi, getArticleApi, reactArticleApi } from 'api/articleApis';
import {
  createAnswerApi,
  deleteAnswerApi,
  reactAnswerApi,
...
import ArticleRedux from './index';

const { actions } = ArticleRedux;

function* newsRootSagas() {
  yield all([
    yield takeLatest(actions.getArticle.type, getArticleSaga),
    yield takeLatest(actions.reactArticle.type, reactArticleSaga),
...
export function* getArticleSaga({
  payload,
}: ReturnType<typeof actions.getArticle>) {
  try {
    const { slug, res, type } = payload;
    const data = yield call(getArticleApi, slug, type);
    if (data) {
...
export function* reactArticleSaga({
  payload,
}: ReturnType<typeof actions.reactArticle>) {
  try {
    yield call(reactArticleApi, payload);
  } catch (error) {
    console.log(error);
  }
}
